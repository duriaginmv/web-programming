...
export const LoginForm = () => {
    ...
    const [key, setKey] = useState({
        API: !window.localStorage.getItem(
            'API-key'
        )
            ? ''
            : window.localStorage.getItem(
        ...
        valid: !!window.localStorage.getItem(
            'API-key'
        ),
    });
    const [
    ...
    function Submit() {
        if (key.API !== null)
            fetch(Url(key.API))
                ...
                .catch((error) => {
                    alert(
                        'Введите действительный API-ключ'
                    );
                    console.log(error);
                });
    }
